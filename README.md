...
1.  구상한걸 기반으로 결제 페이지 만들기
2.  쿠폰 개발하기
3.  mockup data를 만들어 ui에 잘 표시되는지 확인하기 
- - -   
### 쿠폰
1. 기본적으로 가지고 있는 쿠폰중 할인금액이 가장 큰 쿠폰을 사용한다.
    1. 정액제와 정률제 큐에서 첫번째값(최대 할인)을 비교하여 더 큰 쿠폰을 적용시켜준다.
2. 쿠폰이 들어갈 때 제일 빨리 찾아오는 쿠폰의 날짜를 기록해 뒀다가 만약 일주일 안에 만료되는 쿠폰이 있다면, 
쿠폰 교체 버튼 옆이나, 쿠폰 할인금액 및에 알림을 띄워서 알려준다.

### mockup data
json 파일을 만들어 fetch함수로 데이터를 관리할 예정
